...
import logging
logging.basicConfig(filename='/its/home/mt601/kaggle/sartorius-cell-instance-segmentation/script/detectron/astro.log', filemode='w', level=logging.INFO)
logging.info('Start')
import pandas as pd
...
for fold in tqdm(range(1)):
    ...
    evaluator=MAPIOUEvaluator(astro) #
    config=[]
    for threshold in np.arange(0.3, 0.8, 0.1):
        predictors.THRESHOLDS[0]=threshold #
        for iou_thr in [0.3,0.5,0.7]:
            predictors.iou_thr=iou_thr
            for method in ['max']:
                for inverse in [True]:
                    preds=[]
                    for fn in [i['file_name'] for i in astro]: #
    ...
    print(df)
    df.to_csv(f'/its/home/mt601/kaggle/sartorius-cell-instance-segmentation/script/detectron/astro_configs_fold{fold}.csv',index=False)