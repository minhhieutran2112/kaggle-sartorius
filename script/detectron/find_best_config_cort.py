...
import pandas as pd
import logging
logging.basicConfig(filename='/its/home/mt601/kaggle/sartorius-cell-instance-segmentation/script/detectron/cort.log', filemode='w', level=logging.INFO)
logging.info('Start')
...
for fold in tqdm(range(1)):
    ...
    evaluator=MAPIOUEvaluator(cort) #
    config=[]
    for threshold in np.arange(0.3, 0.8, 0.1):
        predictors.THRESHOLDS[1]=threshold #
                        ...
                        for ensemble_merge_thresh_overlap in [0.5,0.75,0.9]:
                                for postprocess in ['after']:
                                    for overlap_first in [False]:
                                        preds1=[]
                                        for fn in [i['file_name'] for i in cort]: #
                                            pred, pred_class = predictors(fn)
                                            preds1.append(pred.pred_masks.cpu().numpy())
    ...
    print(df)
    df.to_csv(f'/its/home/mt601/kaggle/sartorius-cell-instance-segmentation/script/detectron/cort_configs_fold{fold}.csv',index=False)