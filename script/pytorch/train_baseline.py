...
# %%
def get_score(detection,target):
    masks = combine_masks(target['masks'], 0.5)
    labels = pd.Series(detection['labels'].cpu().numpy()).value_counts()
    try:
        mask_threshold = mask_threshold_dict[labels.sort_values().index[-1]]
    except:
        mask_threshold = 1
    pred_masks = combine_masks(get_filtered_masks(detection), mask_threshold)
...
class CompetitionScore(torchmetrics.Metric):
    ...
    def update(self, detections, targets):
        groups=zip(detections,targets)
        for detection,target in groups:
            self.iouscore += get_score(detection,target)
        self.n_records += len(targets)
        print(len(detections),len(targets),self.iouscore,self.n_records)
    def compute(self):
...
class CustomModel(pl.LightningModule):
    def __init__(self, conf):
        ...
        self.model=get_model(self.hparams.num_classes,self.hparams.model_chkpt)
        ...
        self.model.forward=MethodType(model_forward,self.model)
    ...
    def train_dataloader(self):
        return DataLoader(self.ds_train, batch_size=self.hparams.batch_size, shuffle=True, pin_memory=True,
                          num_workers=8, collate_fn=collate_fn)

    def val_dataloader(self):
        return DataLoader(self.ds_val, batch_size=self.hparams.batch_size, shuffle=False, pin_memory=True,
                            num_workers=8, collate_fn=collate_fn)

    def forward(self,batch):
        output = self.model(batch['img'],batch['target'])
        return output
    ...
    def validation_step(self,batch,batch_idx):
        ...
        self.val_metric(detections,batch['target'])
...
class WandbImageCallback(pl.Callback):
    ...
    def on_validation_end(self, trainer, pl_module):
        ...
        for i,img in enumerate(self.ds):
            ...
            result = pl_module.model([image])[1][0]
            masks = combine_masks(target['masks'], 0.5)
            labels = pd.Series(result['labels'].cpu().numpy()).value_counts()
            mask_threshold = mask_threshold_dict[labels.sort_values().index[-1]]
            pred_masks = combine_masks(get_filtered_masks(result), mask_threshold)
